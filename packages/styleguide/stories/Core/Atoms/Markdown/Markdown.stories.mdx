...
There are several overrides available from the `@codecademy/markdown-overrides` package that can also be used.
...
<Canvas>
  <Story name="Codepedia (Override)">
    {(args) => (
      <Markdown
        text='This is an example of an embedded <Codepedia concept="comments" language="js">Codepedia link</Codepedia>. It will render inline for anything that uses the markdown field.'
        overrides={{
          Codepedia: {
            component: Codepedia,
